...
const NotesList = ({onDeleteHandler, onArchivedHandler, datas, searchField}) => {
    ...
    return (
        ...
        <motion.section layout className={unarchivedDatas.length > 0 ? 'grid lg:grid-cols-[repeat(4,_1fr)] grid-rows-[minmax(min-content,_max-content)] md:grid-cols-[repeat(2,1fr)] gap-x-2 gap-y-6 mb-12' : 'text-center text-blue-500 mb-12'}>
            {
        ...
        <motion.section className={archivedDatas.length > 0 ? 'flex flex-wrap gap-x-4 gap-y-6 mb-12' : 'text-center text-blue-500 mb-12'}>
...
const Note = ({onDeleteHandler, onArchivedHandler, data}) => {
    ...
    return(
                <motion.div layout className="md:basis-1/3 lg:basis-1/4 bg-white shadow-xl rounded-md flex overflow-hidden flex-col" key={id}>
